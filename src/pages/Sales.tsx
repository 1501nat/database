...
import { ShoppingCart, Plus, Trash2, DollarSign, Printer } from "lucide-react";
...
const Sales = () => {
  ...
  const fetchData = async () => {
    try {
      const [productsData, customersData, branchesData, employeesData] = await Promise.all([
        supabase.from("san_pham").select("*").eq("trang_thai_sp", "đang KD"),
        supabase.from("khach_hang").select("*"),
        supabase.from("chi_nhanh").select("*"),
        supabase.from("nhan_vien").select("*").eq("trang_thai_lam_viec", "đang làm"),
      ]);
    ...
    if (existingItem) {
      ...
      setCart([
        ...cart,
        {
          ...
          don_gia: product.gia_ban_le || 0,
          so_luong: quantity,
          thanh_tien: (product.gia_ban_le || 0) * quantity,
        },
      ]);
    }
      ...
      cart.map((item) =>
        item.ma_sp === ma_sp
          ? {
              ...
              thanh_tien: newQuantity * item.don_gia,
            }
          : item
      )
    );
  ...
  const handleCheckout = async () => {
    ...
    if (!selectedBranch || !selectedEmployee) {
      toast.error("Vui lòng chọn chi nhánh và nhân viên");
      return;
    }
    ...
    try {
      ...
      // Insert invoice
      const { error: invoiceError } = await supabase.from("hoa_don_ban_hang").insert([
        {
          ma_hdbh: maHDBH,
      ...
      const details = cart.map((item, index) => ({
        ...
        stt_dong: index + 1,
        ma_sp: item.ma_sp,
        so_luong_ban: item.so_luong,
        don_gia_ban: item.don_gia,
      }));
    ...
    <DashboardLayout>
      <div className="p-8">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground mt-1">Tạo hóa đơn bán hàng nhanh chóng</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
                      ...
                      <SelectContent>
                        {products.map((product) => (
                          <SelectItem key={product.ma_sp} value={product.ma_sp}>
                            {product.ten_sp} - {formatCurrency(product.gia_ban_le || 0)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
            ...
            <Card className="shadow-card">
              <CardHeader>
                <CardTitle>Giỏ hàng ({cart.length} sản phẩm)</CardTitle>
              </CardHeader>
              <CardContent>
                {cart.length === 0 ? (
                    ...
                    {cart.map((item) => (
                      <div key={item.ma_sp} className="flex items-center gap-4 p-3 rounded-lg bg-muted/30">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">{formatCurrency(item.don_gia)}</p>
                        </div>
                        <Input
                          type="number"
                          value={item.so_luong}
                          onChange={(e) => updateQuantity(item.ma_sp, parseInt(e.target.value) || 0)}
                          className="w-20"
                          min="0"
                        />
                  ...
                  <Select value={selectedCustomer} onValueChange={setSelectedCustomer}>
                    ...
                    <SelectContent>
                      ...
                      {customers.map((customer) => (
                        <SelectItem key={customer.ma_kh} value={customer.ma_kh}>
                          {customer.ho_ten}
                        </SelectItem>
                      ))}
                    </SelectContent>
