...
const Categories = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...formData,
        ma_dm_super: formData.ma_dm_super || null,
      };
      ...
      if (editingCategory) {
        const { error } = await supabase
          .from("danh_muc_san_pham")
          .update(submitData)
          .eq("ma_dm", editingCategory.ma_dm);
        if (error) throw error;
        toast.success("Cập nhật danh mục thành công!");
      } else {
        const { error } = await supabase.from("danh_muc_san_pham").insert([submitData]);
        if (error) throw error;
  ...
  const handleDelete = async (ma_dm: string) => {
    ...
    try {
      const { error } = await supabase.from("danh_muc_san_pham").delete().eq("ma_dm", ma_dm);
      if (error) throw error;
  ...
  const openEditDialog = (category: Category) => {
    ...
    setFormData({
      ...
      ma_dm_super: category.ma_dm_super || "",
    });
    setDialogOpen(true);
  };

  const getParentName = (ma_dm_super: string | null) => {
    if (!ma_dm_super) return "-";
    const parent = categories.find(c => c.ma_dm === ma_dm_super);
    return parent?.ten_danh_muc || ma_dm_super;
  };
  ...
  const columns = [
    ...
    { header: "Tên danh mục", accessor: "ten_danh_muc" as keyof Category },
    {
      ...
      accessor: ((row: Category) => (
        <Badge variant={row.ma_dm_super ? "outline" : "secondary"}>
          {getParentName(row.ma_dm_super)}
        </Badge>
      )) as any,
    },
    { header: "Mô tả", accessor: ((row: Category) => row.mo_ta || "-") as any },
    {
      ...
      accessor: ((row: Category) => (
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="outline" onClick={() => handleDelete(row.ma_dm)}>
      ...
      <div className="p-8">
                ...
                <div className="grid gap-4 py-4">
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setFormData({ ...formData, ma_dm: e.target.value })} 
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.ma_dm_super} onValueChange={(value) => setFormData({ ...formData, ma_dm_super: value })}>
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        ...
                        {categories
                          .filter(c => c.ma_dm !== formData.ma_dm)
                          .map((cat) => (
                            <SelectItem key={cat.ma_dm} value={cat.ma_dm}>
                              {cat.ten_danh_muc}
                            </SelectItem>
                          ))}
                      </SelectContent>
        ...
        <Card className="shadow-card">
          <CardContent className="p-6">
            <DataTable data={categories} columns={columns} loading={loading} emptyMessage="Chưa có danh mục nào" />
          </CardContent>
        </Card>
      </div>
    </DashboardLayout>
